...
async def on_member_join(member: discord.Member):
    ...
    # ── Clean content — no spoiler spam ──────────────
    await channel.send(
        content=f"@everyone {member.mention} just conquered the gates! ⚔️\n\n💬 Say hi → <#{GENERAL_CHAT_ID}>\n🖥️ Get MC IP → <#{SERVER_INFO_CHANNEL}>\n📺 Subscribe → https://youtube.com/@OPANKUSHFF007",
        embed=embed
    )
